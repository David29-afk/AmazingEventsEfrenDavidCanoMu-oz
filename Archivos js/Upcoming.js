...
function cargarDatosEventos() {
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            eventos = data.events.filter(evento => currentDate <= evento.date); // Filtrar solo eventos futuros
            pintarTargetas(eventos, '', currentDate); // Pintar las tarjetas con eventos futuros
